...
export const myOrders = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${process.env.REACT_APP_PORT}/api/v1/orders/myOrders`);
...
export const getOrderDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${process.env.REACT_APP_PORT}/api/v1/order/${id}`);
...
export const getAllOrders = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${process.env.REACT_APP_PORT}/api/v1/admin/orders`);
...
export const updateOrder = (id, productData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `${process.env.REACT_APP_PORT}/api/v1/admin/order/${id}`,
      productData,
      config
    );
